namespace InSelfLove.Services.Data.Tests
{
    ...
    public class CommentServiceTests
    {
        ...
        public async Task CreateWorksCorrectlyWithValidData()
        {
            ...
            var commentFromDb = await this.commentService.Create(comment, comment.UserId);
            var comment1FromDb = await this.commentService.Create(comment1, comment1.UserId);

            Assert.Equal(1, commentFromDb.Id);
            Assert.Equal(2, comment1FromDb.Id);
        }
        ...
        public async Task CreateThrowsArgumentExceptionWithInvalidData(string userId, string content, int? articleId)
        {
            ...
            await Assert.ThrowsAsync<ArgumentException>(() => this.commentService.Create(comment, comment.UserId));
        }
        ...
        public async Task ArrangeCommentHierarchyWorksFine()
        {
            ...
            await this.commentService.Create(firstLevelSubcomment1, firstLevelSubcomment1.UserId);
        ...
        private async Task<List<Comment>> SeedData()
        {
            ...
            var mainCommentFromDb = await this.commentService.Create(mainComment, mainComment.UserId);

            var firstLevelSubcomment = new Comment
            {
                ParentCommentId = mainCommentFromDb.Id,
                UserId = "2",
                Content = "firstLevelSubcomment",
                ArticleId = 1,
            };
            var firstLevelSubcommentFromDb = await this.commentService.Create(firstLevelSubcomment, mainComment.UserId);

            var secondLevelSubcomment = new Comment
            {
                ParentCommentId = firstLevelSubcommentFromDb.Id,
                UserId = "3",
                Content = "secondLevelSubcomment",
                ArticleId = 1,
            };
            var secondLevelSubcommentFromDb = await this.commentService.Create(secondLevelSubcomment, secondLevelSubcomment.UserId);

            // Third level subcomments don't actully exist as per service logic
            var thirdLevelSubcomment = new Comment
            {
                ParentCommentId = secondLevelSubcommentFromDb.Id,
                UserId = "4",
                Content = "thirdLevelSubcomment",
                ArticleId = 1,
            };
            await this.commentService.Create(thirdLevelSubcomment, thirdLevelSubcomment.UserId);
